...
The dispatch controller is the heart of the distributed, autonomous software factory. It:
...
```bash
# Using npm script
npm run dispatch -- [options]

# Using bun directly
bun src/lib/dispatch/cli-entry.ts [options]
```
...
jobs:
  dispatch:
    ...
    steps:
      ...
      - id: dispatch
        run: |
          bun src/lib/dispatch/cli-entry.ts \
            --max-concurrent 3 \
            --output matrix.json
