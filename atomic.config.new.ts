/**
 * Example Workflow Configuration (New API)
 *
 * This demonstrates the redesigned graph engine API with:
 ...
 * ```bash
 * bun run src/lib/cli.ts run --config atomic.config.new.ts --context '{"issueId": 123}'
 * ```
 */
